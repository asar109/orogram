...
import {
  ...
  extractCountryCodeFromProfileMobile,
  extractCountryNameFromProfileMobile,
  extractMobileNumberFromProfileMobile,
  serverUrl,
...
const AppProfile = () => {
  ...
  const [mobileNumberChange, setMobileNumberChanged] = useState(false);
  ...
  useEffect(() => {
    const c = extractCountryCodeFromProfileMobile(
      userReducer.currentUser.mobile
    );
    const n = extractCountryNameFromProfileMobile(
      userReducer.currentUser.mobile
    );

    setProfile({
      ...profile,
      firstName: userReducer.currentUser.firstName,
      lastName: userReducer.currentUser.lastName,
      mobile: extractMobileNumberFromProfileMobile(
        userReducer.currentUser.mobile
      ),
  ...
  const handelChangeCountryCodeDrop = (e) => {
    setError({ ...error, code: false });
    setProfile({ ...profile, code: e });
  };
  ...
  const updateInfo = async (e) => {
    ...
    // let formattedMobileNumber = profile.mobile

    if (mobileNumberChange && profile.code === "") {
    ...
    if (mobileNumberChange) {
      const {countryCode : cc , countryName : cn} = extractCodeAndCountryNameFromLabel(profile.code.label)

      phone = `${cn} (${cc}) ${profile.mobile}`
    } else {
      phone  = userReducer.currentUser.user.mobile
    }
    ...
    axios
      .patch(`${serverUrl}/users/update`, postData, {
        headers: {
          ...
          <form onSubmit={(e) => updateInfo(e)}>
            ...
            <div className="row">
              <div className="form-group mb-3 col-md-6">
                ...
                <Select
                  value={profile.code}
                  onChange={(e) => handelChangeCountryCodeDrop(e)}
